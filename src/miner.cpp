...
void static YacoinMiner(const CChainParams& chainparams)
{
    ...
    try {
        ...
        while (true) {
            ...
            LogPrintf("YacoinMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
int GenerateYacoins(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&YacoinMiner, boost::cref(chainparams)));
    }
